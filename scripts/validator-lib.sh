...
generate_validator_environment_file() {
    ...
    # Get configuration from environment variables (matches proven run-validator script)
    local arch_data_dir="${ARCH_DATA_DIR:-$home_dir/data/.arch_data}"
    local arch_rpc_bind_ip="${ARCH_RPC_BIND_IP:-127.0.0.1}"
    local arch_rpc_bind_port="${ARCH_RPC_BIND_PORT:-9002}"
    local arch_titan_endpoint="${ARCH_TITAN_ENDPOINT:-https://titan-public-http.test.arch.network}"
    local arch_titan_socket_endpoint="${ARCH_TITAN_SOCKET_ENDPOINT:-titan-public-tcp.test.arch.network:3030}"
    local arch_network_mode="${ARCH_NETWORK_MODE:-testnet}"

    log_echo "Generating validator environment file..."

    # Create environment file (matches proven run-validator script)
    sudo tee "$env_file" >/dev/null <<EOF
# Arch Network Validator Configuration
# Generated by validator-lib.sh (matches proven run-validator script)

# Validator Configuration
ARCH_DATA_DIR=$arch_data_dir
ARCH_RPC_BIND_IP=$arch_rpc_bind_ip
ARCH_RPC_BIND_PORT=$arch_rpc_bind_port
ARCH_TITAN_ENDPOINT=$arch_titan_endpoint
ARCH_TITAN_SOCKET_ENDPOINT=$arch_titan_socket_endpoint
ARCH_NETWORK_MODE=$arch_network_mode
EOF
...
update_validator_operator() {
    ...
    generate_validator_environment_file "$username"
